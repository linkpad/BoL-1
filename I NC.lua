...
local  BlitzcrankAssGrabber_Version = 1.9
...
function Combo(unit)
	if ValidTarget(unit) and unit ~= nil and unit.type == myHero.type then
		if not Settings.Combo.useQ then
			CastQ(unit)
		end
		if Seings.Combo.UseQ then
			CastQ2(unit)
		end
		CastE(unit)
...
function CastQ2(unit)
	if unit ~= nil and GetDistance(unit) <= SkillQ.range and GetDistance(unit) >= Settings.Combo.qRange and SkillQ.ready then
		CastPosition,  HitChance,  Position = VP:GetLineCastPosition(unit, SkillQ.delay, SkillQ.width, SkillQ.range, SkillQ.speed, myHero, true)	
		if HitChance >= 2 then
			Packet("S_CAST", {spellId = _Q, fromX = CastPosition.x, fromY = CastPosition.z, toX = CastPosition.x, toY = CastPosition.z}):send()
		end
...
function Menu()
	...
	Settings:addSubMenu("["..myHero.charName.."] - Combo Settings", "combo")
		...
		Settings.combo:addParam("useQ", "Use (Q) only if enemy is not closest", SCRIPT_PARAM_ONOFF, true)
		Settings.combo:addParam("qRange", "Use (Q) if range > X", SCRIPT_PARAM_SLICE, 350, 100, 800, 0)
	...
	Settings.extra:addParam("autoE", "Auto use (E) if (Q) hit", SCRIPT_PARAM_ONOFF, true)
